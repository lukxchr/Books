...
from flask import Flask, jsonify, flash, render_template, request, session, redirect, url_for
...
def book(book_id):
	...
	if form.validate_on_submit():
		if review_by_current_user:
			...
			review = Review(title=form.title.data, content=form.content.data, rating=int(form.rating.data), user_id=int(current_user.id), book_id=int(book_id))
	...
	goodreads_rating, goodreads_ratings_count = get_goodreads_details(book.isbn, Config.GOODREADS_API_KEY)
	...
	book_details = dict(goodreads_rating=goodreads_rating, goodreads_ratings_count=goodreads_ratings_count,
		description=google_description, cover=google_cover)
	return render_template("book.html", book=book, form=form, reviews=reviews, review_by_current_user=review_by_current_user, book_details=book_details)
...
def book_json(isbn):
	...
	try:
		...
		print(book)
		goodreads_rating, goodreads_ratings_count = get_goodreads_details(book.isbn, "ow2ypDLGjjsQgTc2etsz7w")
		print(book, goodreads_rating, goodreads_ratings_count)
		return jsonify({
			"title": book.title,
			"author": book.author,
			"year": book.year,
...
def get_goodreads_details(isbn, api_key):
	try:
		...
		goodreads_id = res.json()['books'][0]['id']
